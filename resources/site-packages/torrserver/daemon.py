...
def download_binary(version, binary, path):
    filedata = urllib2.urlopen('https://github.com/YouROK/TorrServer/releases/download/{0}/{1}'.format(version, binary))
    CHUNK_SIZE = 8192
    bytes_read = 0
    size = int(filedata.info().getheaders("Content-Length")[0])
    pDialog = xbmcgui.DialogProgressBG()
    pDialog.create('TorrServer Binary', 'Downloading {0} [{1} Mb]'.format(binary, (size/1024/1024)))
    with open(path, 'wb') as f:
        while True:
            ...
            if prc > 100:
                prc = 100
            pDialog.update(prc, message='Downloading {0} [{1} Mb]'.format(binary, (size/1024/1024)))
        pDialog.close()
