...
from torrserver.util import notify, system_information, getLocalizedString, getShortPath, dialog_yesno
...
def shutdown():
    try:
        ...
        log.error("Can't shutdown server with error: {0}".format(e))

def download_binary(version, binary, path):
    filedata = urllib2.urlopen('http://github.com/YouROK/TorrServer/releases/download/{0}/{1}'.format(version, binary))
    CHUNK_SIZE = 8192
    bytes_read = 0
    size = int(filedata.info().getheaders("Content-Length")[0])
    pDialog = xbmcgui.DialogProgressBG()
    pDialog.create('Загрузка {0} [{1} Мб]'.format(binary, (size/1024/1024)))
    with open(path, 'wb') as f:
        while True:
            ...
            if prc > 100:
                prc = 100
            pDialog.update(prc, message='Загрузка {0} [{2}/{1} Mb]'.format(binary, (size/1024/1024), (bytes_read/1024/1024)))
        pDialog.close()
...
def start_torrserverd(**kwargs):
    ...
    database_path = getShortPath(os.path.join(xbmc.translatePath(ADDON.getAddonInfo("profile")).decode('utf-8')))
...
def get_torrserver_binary():
    ...
    if not os.path.exists(binary_path) or ADDON.getSetting("update_binaries") == "true":
        ...
        download = dialog_yesno("LOCALIZE[30000];;" + "TorrServer-%(os)s-%(arch)s" % PLATFORM + " v%s" % version, "LOCALIZE[30007]")
        system_information()
        if download:
            try:
        ...
        elif dialog_yesno("Тогда возможно попробуем открыть его из папки ??"):
            bFile = xbmcgui.Dialog().browseSingle(1, 'Выберете файл сервера '+binary, 'files', binary)
            if bFile:
