...
def notify(message, header=ADDON_NAME, time=5000, image=ADDON_ICON):
    ...
    return dialog.notification(toUtf8(header), toUtf8(message), toUtf8(image), time)
...
def dialog_yesno(message, message1="", header=ADDON_NAME):
    dialog = xbmcgui.Dialog()
    return dialog.yesno(getLocalizedLabel(header), getLocalizedLabel(message), getLocalizedLabel(message1))
